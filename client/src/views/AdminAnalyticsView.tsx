      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '24px', marginBottom: '32px' }}>
        <div className="card">
          ...
          <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '16px' }}>
            Comparative monthly breakdown of student event participation
          </p>
        ...
        <div className="card">
          <h3 style={{ fontSize: '16px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <ShieldCheck color="#10b981" /> Department Participation
          </h3>
          ...
          {departmentStats.map((dept) => (
            <div key={dept.department}>
              ...
              <div style={{ height: '8px', background: 'var(--border-color)', borderRadius: '99px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.min(dept.registrations * 18, 100)}%`, background: 'linear-gradient(90deg, #6366f1, #10b981)', borderRadius: '99px' }} />
              </div>
            </div>
          ))}
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color)', textAlign: 'left', color: 'var(--text-muted)' }}>
                <th style={{ padding: '10px' }}>Organizer Name</th><th style={{ padding: '10px' }}>Email</th><th style={{ padding: '10px' }}>Events Hosted</th><th style={{ padding: '10px' }}>Total Signups</th><th style={{ padding: '10px' }}>Total Attendees</th><th style={{ padding: '10px' }}>Avg Attendance Rate</th>
              </tr>
            </thead>
            <tbody>
              {leaderboard.map(org => (
                <tr key={org.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  <td style={{ padding: '12px 10px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 600 }}><img src={org.avatar_url} alt={org.name} style={{ width: '32px', height: '32px', borderRadius: '50%' }} />{org.name}</td>
                  <td style={{ padding: '12px 10px' }}>{org.email}</td><td style={{ padding: '12px 10px', fontWeight: 700 }}>{org.events_hosted}</td><td style={{ padding: '12px 10px' }}>{org.total_registrations}</td><td style={{ padding: '12px 10px' }}>{org.total_attendees}</td><td style={{ padding: '12px 10px' }}><span className="badge badge-success">{org.avg_attendance_rate}%</span></td>
                </tr>
              ))}
            </tbody>
          </table>
